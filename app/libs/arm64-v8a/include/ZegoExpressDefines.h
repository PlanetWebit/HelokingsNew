...
struct ZegoPlayerConfig {
    ...
    ZegoPlayerConfig()
        : resourceMode(ZEGO_STREAM_RESOURCE_MODE_DEFAULT), cdnConfig(nullptr),
          videoLayer((ZegoPlayerVideoLayer)99), roomID(""),
...
struct ZegoMixerImageInfo {
    ...
    ZegoMixerImageInfo(std::string url) : url(url) {}
};
...
struct ZegoMixerTask {
    ...
    ZegoMixerTask(std::string taskID = "")
        : taskID(taskID), watermark(nullptr), backgroundColor(0), enableSoundLevel(false),
          streamAlignmentMode(ZEGO_STREAM_ALIGNMENT_MODE_NONE), userData(nullptr),
          userDataLength(0), minPlayStreamBufferLength(-1) {}
};
