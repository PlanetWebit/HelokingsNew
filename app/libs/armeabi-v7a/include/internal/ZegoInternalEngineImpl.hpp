...
class ZegoExpressEngineImp : public IZegoExpressEngine {
  public:
    ...
    std::vector<ZegoScreenCaptureSourceInfo> getScreenCaptureSources(int thumbnailWidth,
        ...
        oInternalOriginBridge->freeScreenCaptureSourceList(source_info_list, count);
        return result;
    }
    ...
    IZegoMediaDataPublisher *createMediaDataPublisher() override {
        auto index = oInternalOriginBridge->createMediaDataPublisher();
        if (index < 0) {
            return nullptr;
        }

        auto media_data_publisher_instance =
            std::make_shared<ZegoExpressMediaDataPublisherImp>(index);
    ...
    void destroyMediaDataPublisher(IZegoMediaDataPublisher *&mediaDataPublisher) override {
        if (mediaDataPublisher) {
            oInternalOriginBridge->destroyMediaDataPublisher(0);
            oInternalCallbackCenter->eraseZegoExpressMediaDataPublisherImp(0);
        }
    }
